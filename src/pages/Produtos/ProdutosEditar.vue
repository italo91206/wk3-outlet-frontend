  ...
  <v-main class="pa-12">
    <v-container>
          ...
          <v-form v-model="isValidForm" ref="form" @submit.prevent>
            ...
            <v-row id="imagesRow">
              ...
              >
                ...
                ></v-img>
                <v-btn
                  class="deletar-imagem"
                  color="error"
                  @click="removerImagem(imagem.imagem_id)"
                  >Remover</v-btn
                >
              </v-col>
            </v-row>
      ...
      <v-row class="float-right">
        ...
        <v-btn @click="deletarConfirm" color="error" class="mr-2"
          >Deletar produto</v-btn
        >
        <v-btn
          @click="salvarProduto"
    ...
    <Helper>
      <template #titulo> Editar produto </template>

      <template #texto>
        <p>
          O referente setor traz a edição de um produto já existente,
          possibilitando ao administrador editar os seguintes itens: as imagens
          do produto; nome do produto; o SKU; o preço; custo; a possibilidade em
          estoque; peso em gramas; a categoria do produto aderido ao setor e
          construído pelo administrador; o modelo e a marca constatados em
          seleções construídas pelo gestor.<br /><br />O produto também pode
          constar com uma descrição oferecida pelo gestor, que pode oferecer
          variações da mercadoria em relevância, adicionando cor, tamanho e
          quantidade.<br />Ao final, o administrador tem a possibilidade de
          salvar o material para integrá-lo ou voltar a páginas de consultas.
        </p>
      </template>
    </Helper>
  </v-main>
...
export default {
  ...
  methods: {
    ...
    async salvarProduto() {
      ...
      if (this.isValidForm) {
        ...
        const response = await produtoService.atualizarProduto(
          this.produtoToPost
        );
        if (response.data.success) {
    ...
    async getProduto() {
      ...
      if (response.data.success) this.produtoToPost = response.data.data;
      else {
        this.$toast.error(response.data.message);
        this.$router.push("/produtos");
      }
    ...
    async deletarProduto() {
      const response = await produtoService.deletarProduto(
        this.produtoToPost.produto_id
      );
      if (response.data.success) {
    ...
    async carregarMotivos() {
      ...
      if (response.data.success) this.motivos = response.data.data;
      else {
        this.$toast.error(response.data.message);
        this.$router.push("/produtos");
      }
    ...
    async carregarUrlProdutoPai(id) {
      ...
      if (response.data.success) this.link_pai = response.data.data.url;
    ...
    async removerImagem(id) {
      ...
      if (!response.data.success) this.$toast.error(response.data.message);
        ...
        let filtrado = this.produtoToPost.imagens.filter((imagem) => {
          return imagem.imagem_id != id;
        });
    ...
    deletarConfirm() {
      const acao = confirm(
        "Deseja mesmo remover o produto ? Esta ação é irreversível !"
      );
      if (acao) this.deletarProduto();
    },
    ...
    async removerVariacao(to_delete) {
      ...
      if (response.data.success) {
        ...
        this.variacoes.forEach((variacao) => {
          if (variacao.variacao_id == to_delete) achei = index;
          index++;
        });
  ...
  watch: {
    produtoToPost: function () {
      if (this.produtoToPost.modelo_id)
        this.modeloSelecionado = this.produtoToPost.modelo_id;
      if (this.produtoToPost.marca_id)
        this.marcaSelecionado = this.produtoToPost.marca_id;
      if (this.produtoToPost.produto_pai)
        this.carregarUrlProdutoPai(this.produtoToPost.produto_pai);
      if (this.produtoToPost.categoria_id)
        this.categoriaSelecionado = this.produtoToPost.categoria_id;
